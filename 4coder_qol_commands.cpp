...
CUSTOM_DOC("Automatically open and close curly braces.")
{
...
CUSTOM_DOC("Automatically open and close brackets.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    write_text(app, string_u8_litexpr("[]"));
    view_set_cursor(app, view, seek_pos(view_get_cursor_pos(app, view) - 1));
...
CUSTOM_COMMAND_SIG(write_parenthesis)
CUSTOM_DOC("Automatically open and close parenthesis.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    write_text(app, string_u8_litexpr("()"));
    view_set_cursor(app, view, seek_pos(view_get_cursor_pos(app, view) - 1));
...
CUSTOM_DOC("Automatically open and close single quotes.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    write_text(app, string_u8_litexpr("''"));
    view_set_cursor(app, view, seek_pos(view_get_cursor_pos(app, view) - 1));
...
CUSTOM_DOC("Automatically open and close double quotes.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    write_text(app, string_u8_litexpr("\"\""));
    view_set_cursor(app, view, seek_pos(view_get_cursor_pos(app, view) - 1));
...
CUSTOM_COMMAND_SIG(backspace_matching_pairs)
CUSTOM_DOC("Automatically remove empty matching operand.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadWrite);
    i64 pos = view_get_cursor_pos(app, view);
    pos = Max(pos, 1);
    char before = buffer_get_char(app, buffer, pos - 1);
    char after = buffer_get_char(app, buffer, pos);
    b32 delete_pair = false;
    b32 inside_pair = false;
    if (pos >= 2) {
        char c = buffer_get_char(app, buffer, pos - 2);
        delete_pair = (before == '}' && c == '{')
            || (before == ']' && c == '[')
            || (before == ')' && c == '(')
            || (before == '\'' && c == '\'')
            || (before == '"' && c == '"');
    ...
    buffer_replace_range(app, buffer, Ii64(delete_start, delete_end), string_u8_litexpr(""));
}

CUSTOM_COMMAND_SIG(delete_matching_pairs)
CUSTOM_DOC("Automatically remove empty matching operand.")
{
    View_ID view = get_active_view(app, Access_ReadWrite);
    Buffer_ID buffer = view_get_buffer(app, view, Access_ReadWrite);
